...
def categorize_dish(dish_name, dish_ingredients):
    ...
    """
    if not dish_ingredients.difference(VEGAN):
        return f"{dish_name}: VEGAN"
    if not dish_ingredients.difference(VEGETARIAN):
        return f"{dish_name}: VEGETARIAN"
    if not dish_ingredients.difference(PALEO):
        return f"{dish_name}: PALEO"
    if not dish_ingredients.difference(KETO):
...
def singleton_ingredients(dishes, _):
