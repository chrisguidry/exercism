...
class CardGamesTest(unittest.TestCase):
    ...
    def test_get_rounds(self):
        ...
        for variant, (number, rounds) in enumerate(zip(input_vars, results), start=1):
            error_message = (
                f"Expected rounds {rounds} given the current round {number}."
            )
    ...
    def test_concatenate_rounds(self):
        ...
        results = [[], [0, 1], [1, 2], [1, 2], [27, 28, 29, 35, 36], [1, 2, 3, 4, 5, 6]]

        for variant, ((rounds_1, rounds_2), rounds) in enumerate(
            zip(input_vars, results), start=1
        ):
            error_message = (
                f"Expected {rounds} as the concatenation of {rounds_1} and {rounds_2}."
            )
            with self.subTest(
                f"variation #{variant}", input=(rounds_1, rounds_2), output=rounds
            ):
                self.assertEqual(
                    rounds, concatenate_rounds(rounds_1, rounds_2), msg=error_message
                )
    ...
    def test_list_contains_round(self):
        ...
        for variant, ((rounds, round_number), contains) in enumerate(
            zip(input_vars, results), start=1
        ):
            error_message = (
            ...
            with self.subTest(
                f"variation #{variant}", input=(rounds, round_number), output=contains
            ):
                self.assertEqual(
                    contains,
    ...
    def test_approx_average_is_average(self):
        ...
        for variant, (hand, same) in enumerate(zip(input_vars, results), start=1):
            ...
            with self.subTest(f"variation #{variant}", input=hand, output=same):
                self.assertEqual(
                    same, approx_average_is_average(hand), msg=error_message
                )
    ...
    def test_average_even_is_average_odd(self):
        ...
        for variant, (hand, same) in enumerate(zip(input_vars, results), start=1):
            ...
            with self.subTest(f"variation #{variant}", input=hand, output=same):
                self.assertEqual(
                    same, average_even_is_average_odd(hand), msg=error_message
                )
    ...
    def test_maybe_double_last(self):
        ...
        for variant, (hand, doubled_hand) in enumerate(
            zip(input_vars, results), start=1
        ):
            error_message = (
                f"Expected {doubled_hand} as the maybe-doubled version of {hand}."
            )
